...
6. **Duplicate pre-commit Unicode lint command** (2026-08-18): The repo-local lefthook fragment repeated the command already supplied by the shared fragment set, causing YAML parsing and fidelity checks to fail. The redundant pre-commit command was removed.
7. **Stale lefthook materialization** (2026-08-18): The repo-local fragment retained a redundant pre-push Unicode command, so the generated hook configuration drifted from the standard fragment set. The local fragment was reduced to metadata-only, and the missing actionlint wrapper was added to the flake package set.
