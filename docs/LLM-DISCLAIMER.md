...
sections: `B D I T V`. Where section B is present it is the defect log:
...
This repository is gated by its Lefthook configuration. Before a
machine-authored branch is pushed, it is run against that gate: the same checks
a human gets on
